    ...
    while q:
        ...
        for v in graph.neighbors(u):
            if v not in node2distances:
                ...
                if node2distances[v] == node2distances[u]+1:
                    node2parents[v].append(u)
                    node2num_paths[v] += 1
...
def complexity_of_bfs(V, E, K):
    ...
    True
    """
    #considering worst case where every node is connected to every other node in graph
    return 1.*(V * V)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    def parentHasChild(parent):
        if parent in [x for v in node2parents.values() for x in v]:
            return True
        else:
            return False

    def getChilds(parent):
        return [ k for k,v in node2parents.items() if parent in v ]

    def getEdgeValues(parent):
        value = 0.0
        for child in getChilds(parent):
            value += result[tuple(sorted((child,parent)))]
        return value

    def calculateEdgeValues(parent):
        if(node2num_paths[parent] == 1):
            result[tuple(sorted((''.join(node2parents[parent]),parent)))]=nodeValue[parent]
        elif(node2num_paths[parent] > 1):
            #print("in SP else")
            totalSP = 0.0
            for p in node2parents[parent]:
                totalSP += node2num_paths[p]
            for p in node2parents[parent]:
                result[tuple(sorted((p,parent)))]=node2num_paths[p]/totalSP

    def rec_bottom_up(parent):
        #print ("inside bottom_up with parent: "+parent)
        if(parentHasChild(parent)):
            #print ("working for root: "+parent)
            #childs = getChilds(parent)
            #print ("has childs: "+str(childs))
            for everyChild in getChilds(parent):
                if not everyChild in nodeValue:
                    rec_bottom_up(everyChild)

            if parent == root:
                return result
            else:
                #print ("in else and calculating stuff for : "+parent)
                edgeValue = getEdgeValues(parent)
                nodeValue[parent] = 1.0 + edgeValue
                calculateEdgeValues(parent)
        else:
            #print ("end calculating stuff for node: "+parent)
            if not (parentHasChild(parent)):
                nodeValue[parent] = 1.0
            calculateEdgeValues(parent)
...
def approximate_betweenness(graph, max_depth):
    ...
    rd = dict(result)
    rd.update((k, v * 0.5) for k,v in rd.items())
    #print (sorted(rd.items()))
    return rd
...
def partition_girvan_newman(graph, max_depth):
    ...
    just remove edges un
    til more than one component is created, then return
    those components.
    ...
    re_list = (sorted(ab.items(), key=lambda x: (-x[1],x[0])))
    #print("list is: "+str(re_list))
    components = [c for c in nx.connected_component_subgraphs(G)]
    cnt=0
    while len(components) == 1:
        re = re_list[cnt][0]
        #print ("removign "+str(re))
        G.remove_edge(*re)
        components = [c for c in nx.connected_component_subgraphs(G)]
        cnt+=1
...
def get_subgraph(graph, min_degree):
    ...
    """
    deg = graph.degree()
    d = [n for n in deg if deg[n] >= min_degree]
    return graph.subgraph(d)
...
def cut(S, T, graph):
    ...
    for i in range(len(S)):
        for k in graph.edges():
            if k[0]==S[i] or k[1]==S[i]:
                SL.append(k)

    for i in range(len(T)):
        for k in graph.edges():
            if k[0]==T[i] or k[1]==T[i]:
                TL.append(k)
...
def norm_cut(S, T, graph):
    ...
    """
    return ((1.0*cut(S,T,graph)/(volume(S,graph))) + (1.0*cut(S,T,graph)/(volume(T,graph))))
...
def score_max_depths(graph, max_depths):
    ...
    result = []
    for i in max_depths:
        #print ("cal for i:"+str(i))
        components = partition_girvan_newman(graph,i)
        #print ("len of 1 c"+str(len(components[0].nodes())))
        #print ("len of 2 c"+str(len(components[1].nodes())))
        nc = norm_cut(list(set(components[0].nodes())),list(set(components[1].nodes())),graph)
        #print ("nc is "+str(nc))
        result.append((i,nc))
    return result
...
def make_training_graph(graph, test_node, n):
    ...
    """
    G_train = graph.copy()
    edges_to_rem = sorted(graph.edges(test_node))[:n]
...
def jaccard(graph, node, k):
    ...
    """
    scores = []
    nodeNi = set(graph.neighbors(node))
    for n in graph.nodes():
        if n != node and not graph.has_edge(node, n):
            nodeNj = set(graph.neighbors(n))
            score = 1. * (len(nodeNi & nodeNj)) / (len(nodeNi | nodeNj))
...
def path_score(graph, root, k, beta):
    ...
    for n in graph.nodes():
        if n != root and not graph.has_edge(root, n):
            a,b,c = bfs(graph,root,math.inf)
            score = math.pow(beta,a[n]) * b[n]
